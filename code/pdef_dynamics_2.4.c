...
Version 2.4
last edit: 22.01.2016 by Christian

authors: Christian Guill & Florian D. Schneider

comments:	program has to be invoked with a parameter (two-digit number, i.e., 01)
			updated version for revision process
			V. 2.2.1: es kann eingestellt werden, wie viele reine Herbivoren bzw. Praedatoren es geben soll.
			V. 2.4: version to work with on the iDiv-meeting (25 - 27 Jan. 2016)

to do:		not much, but write better change log.
...
int seed=450000;						// seed for random number generator
...
double zeta_c=6;//7:					// log_10 of the mean of the consumer (animal) body masses
double sigma_c=3;//2.5;				// width of the distribution of consumer (animal) body masses
...
double sigma_b=3;//2.5;				// width of the distribution of basal (plant) body masses
...
double mean_h_c = -0.48;		//
double sigma_h_c = 0.03;//0.2;		//
double h_p;					// exponent for predy body-mass scaling of the handling time (value is drawn from distribution)
double mean_h_p = -0.66;		//
double sigma_h_p = 0.02;//0.2;		//

double hill;					// Hill coefficient (value is drawn from distribution
double mu_hill=1.5;			// 
double sigma_hill=0.2;		// 
double C_interference;		// interference competition (value is drawn from distribution)
double mu_Cint=0.8;			// 
double sigma_Cint=0.2;		// 
...
double D_N=0.25;//0.25						// nutrient turnover rate
...
//	********** calculate trophic levels **********
 	double *TLmin=(double *) calloc(S,sizeof(double));		// Memory for trophic level (Def: distance)
	double *TLav=(double *) calloc(S,sizeof(double));		// Memory for trophic level (Def: mean)
	double *TLeff=(double *) calloc(S,sizeof(double));		// Memory for trophic level (Def: effective)
	...
	Min_TL(Ap,TLmin);										// get initial minimal trophic level 
	Average_TL(Ap,TLav,TLmin);								// get initial mean trophic level 
	...
	meanTLmin_ini=meanTLmin_ini/S_c;						// mean initial min. trophic level of consumers
	meanTLav_ini=meanTLav_ini/S_c;							// mean initial mean trophic level of consumers
	...
	Min_TL(defmat,TLmin);									// get final min. trophic level 
	Average_TL(defmat,TLav,TLmin);							// get final mean trophic level 
	Eff_TL(defmat,meanB,params,TLeff,TLmin);				// get effective trophic level 
	...
	if(persistence_c!=0)
	{
		meanTLmin_fin=meanTLmin_fin/(S_c*persistence_c);	// average final min. trophic level of consumers
		meanTLav_fin=meanTLav_fin/(S_c*persistence_c);		// average final mean trophic level of consumers
		meanTLeff=meanTLeff/(S_c*persistence_c);			// average effective trophic level of consumers
	}
	else
	{
		meanTLmin_fin=0;
...
//	Links,  masses-, vulnerability- and generality-distribution
	...
	for(i=0;i<S;i++)
	{
		...
		gen[i]=gsl_blas_dasum(&tempp_in.vector);							// generality distribution
	...
	for(i=0;i<S_b;i++)
	{
		inivul_b+=vul[i]/S_b;												// mean initial vulnerability of basal species
		inilogmas_b+=logmas[i]/S_b;											// mean initial log. bodymass of basal species
		inidens_b+= iniB[i]/gsl_vector_get(mas,i);							// initial total abundance of basal species 
	}
	for(i=0;i<S_c;i++)
	{
		...
		inilogmas_c+=logmas[S_b+i]/S_c;										// mean initial log. bodymass  of consumers
		inidens_c+= iniB[S_b+i]/gsl_vector_get(mas,S_b+i);					// initial total abundance  of consumers
	}
	...
	iniSDvul_b=sqrt(iniSDvul_b);											// SD initial vulnerability of basal species
	iniSDlogmas_b=sqrt(iniSDlogmas_b);										// SD initial log. bodymass of basal species
	...
	iniSDlogmas_c=sqrt(iniSDlogmas_c);										// SD initial log. bodymass  of consumers
...
//	finale Groessen:
	...
	for(i=0;i<S;i++)
	{
		...
		vul[i]=gsl_blas_dasum(&tempp_out.vector);							// out-degree distribution
	}
	...
	for(i=0;i<S_b;i++)
	{
		if(B[i]>=1e-6)
		{
			finvul_b+=vul[i]/(S_b*persistence_b);							// mean final vulnerability of basal species
			finlogmas_b+=logmas[i]/(S_b*persistence_b);						// mean final log. bodymass of basal species
			findens_b+= meanB[i]/gsl_vector_get(mas,i);						// final total abundance of all basal species
		}
	}
	for(i=0;i<S_c;i++)
	{
		if(B[S_b+i]>=1e-6)
		{
			finvul_c+=vul[S_b+i]/(S_c*persistence_c);						// mean final vulnerability of consumers
			fingen_c+=gen[S_b+i]/(S_c*persistence_c);						// mean final generality of consumers
			finlogmas_c+=logmas[S_b+i]/(S_c*persistence_c);					// mean final log. bodymass of consumers
			findens_c+= meanB[S_b+i]/gsl_vector_get(mas,S_b+i);				// finale total abundance of all consumers
		}
	}
...
double functional_diversity(gsl_vector *mas)
{
	...
	for(j=0; j<1000; j++)				// get correction term
	{
		s = (j+0.5) * 0.002;
		M_prey = pow(10,s);
